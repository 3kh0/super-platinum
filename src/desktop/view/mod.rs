...
pub fn shell() -> Element {
    ...
    let show_timeline = match snapshot.main_view {
        ...
        // Activity opens threads in the side pane; main stays empty until a channel is opened.
        MainView::Activity => snapshot.activity_detail_open || snapshot.thread_root.is_some(),
        ...
        main {
            ...
            if let Some(root) = snapshot.thread_root.as_ref() {
                aside { class: "thread-pane",
                    header {
                        h2 { "Thread" }
                        button { onclick: move |_| state.write().thread_root = None, "×" }
                    }
